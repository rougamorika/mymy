...
FIXED_LEFT_TRAY_RECT_PIXELS = (170, 78, 216, 396)
...
RECTILINEAR_REPAIR_LABELS = {"square", "rectangle", "cross"}
...
def _mask_from_contour(contour: np.ndarray, padding: int = 4) -> Tuple[np.ndarray, Tuple[int, int]]:
    x, y, w, h = cv2.boundingRect(contour.astype(np.int32))
    mask = np.zeros((h + padding * 2, w + padding * 2), dtype=np.uint8)
    shifted = contour.astype(np.int32) - np.array([[[x - padding, y - padding]]], dtype=np.int32)
    cv2.drawContours(mask, [shifted], -1, 255, -1)
    return mask, (x - padding, y - padding)
...
def _box_contour(x: int, y: int, w: int, h: int) -> np.ndarray:
    return np.array(
...
class ShapeRecognizer:
    ...
    def _repair_rectilinear_contour(self, contour: np.ndarray, result: Dict[str, object]) -> tuple[np.ndarray, Dict[str, object]]:
        raw_label = str(result["raw_label"])
        features = result["features"]
        if raw_label != "four_leaf":
            return contour, result

        bbox_x, bbox_y, bbox_w, bbox_h = features["bounding_rect"]
        ...
        if defect_count > 10:
            return contour, result
        if not (0.45 <= extent <= 0.92):
            return contour, result
        if aspect_ratio < 0.55 or aspect_ratio > 2.2:
            return contour, result
        if float(features["solidity"]) < 0.93:
            return contour, result

        mask, origin = _mask_from_contour(contour, padding=4)
        close_kernel = np.ones((3, 3), dtype=np.uint8)
        repaired = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, close_kernel)
        repaired = cv2.dilate(repaired, close_kernel, iterations=1)
        repaired_contour = _largest_contour(repaired)
        candidates: list[tuple[np.ndarray, Dict[str, object]]] = []
        if repaired_contour is not None:
            repaired_contour = repaired_contour + np.array([[[origin[0], origin[1]]]], dtype=np.int32)
            repaired_result = self._classify_contour_base(repaired_contour)
            candidates.append((repaired_contour, repaired_result))

        box_contour = _box_contour(bbox_x, bbox_y, bbox_w, bbox_h)
        box_result = self._classify_contour_base(box_contour)
        candidates.append((box_contour, box_result))

        for candidate_contour, candidate_result in candidates:
            if str(candidate_result["raw_label"]) in RECTILINEAR_REPAIR_LABELS:
                return candidate_contour, candidate_result
    ...
    def _detect_from_mask(self, frame: np.ndarray, mask: np.ndarray) -> List[Dict[str, object]]:
        ...
        for contour in contours:
            ...
            if x <= MAX_BORDER_TOUCH + 2 or y <= MAX_BORDER_TOUCH + 2:
                continue
            if x + w >= frame.shape[1] - (MAX_BORDER_TOUCH + 2) or y + h >= frame.shape[0] - (MAX_BORDER_TOUCH + 2):
                continue
            extent = area / float(w * h) if w and h else 0.0
            if area > 5000.0 and extent < MIN_EXTENT_FOR_LARGE_CONTOURS:
                continue
            result = self.classify_contour(contour)
            contour, result = self._repair_rectilinear_contour(contour, result)
            moments = cv2.moments(contour)
            if moments["m00"] == 0:
                continue
            x, y, w, h = cv2.boundingRect(contour)
    ...
    def _classify_contour_base(self, contour: np.ndarray) -> Dict[str, object]:
    ...
    def classify_contour(self, contour: np.ndarray) -> Dict[str, object]:
        result = self._classify_contour_base(contour)
        _, repaired_result = self._repair_rectilinear_contour(contour, result)
